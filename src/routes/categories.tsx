...
const categoryPresentation: Record<Categorie, { icon: typeof Cpu; color: string }> = {
  ...
  Apple: {
    icon: Smartphone,
    color: "bg-slate-100 text-slate-600 dark:bg-slate-900/50 dark:text-slate-300",
  },
  Drivers: {
    icon: HardDrive,
...
function CategoriesPage() {
    ...
    return (
      <div className="mx-auto max-w-6xl">
        <EmptyState
          ...
          description="Créez ou sélectionnez une boutique pour consulter ses catégories d'outils."
        />
        ;
      </div>
    );
  ...
  if (error) {
    return (
      <div className="mx-auto max-w-6xl rounded-xl border border-destructive/40 bg-destructive/10 p-4 text-sm text-destructive">
    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        ...
        <p className="mt-2 text-muted-foreground">
          Explorez le catalogue de cette boutique par famille d'outils.
        </p>
      </div>
