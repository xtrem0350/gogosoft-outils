...
function ParametresPage() {
  ...
  const [profile, setProfile] = useState<{ full_name: string | null; email: string | null; phone?: string | null }>({ full_name: "", email: "", phone: "" });
  const [shop, setShop] = useState<{ id: string; name: string; address: string | null; phone: string | null } | null>(null);
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (nextProfile) {
        setProfile({ full_name: nextProfile.full_name, email: nextProfile.email, phone: (nextProfile as { phone?: string | null }).phone ?? null });
      }
      ...
      if (currentShop) {
        setShop({ id: currentShop.id, name: currentShop.name, address: currentShop.address ?? null, phone: currentShop.phone ?? null });
      }
  ...
  async function saveProfile() {
    ...
    await updateProfile(userId, { full_name: profile.full_name, email: profile.email ?? undefined, phone: profile.phone ?? null } as never);
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <Tabs value={tab} onValueChange={setTab} className="w-full">
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="full-name" value={profile.full_name ?? ""} onChange={(e) => setProfile((current) => ({ ...current, full_name: e.target.value }))} className="pl-9" />
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="profile-phone" value={profile.phone ?? ""} onChange={(e) => setProfile((current) => ({ ...current, phone: e.target.value }))} className="pl-9" />
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="profile-email" type="email" value={profile.email ?? ""} onChange={(e) => setProfile((current) => ({ ...current, email: e.target.value }))} className="pl-9" />
                </div>
              </div>
              <Button onClick={() => void saveProfile()}><Save className="size-4" />Enregistrer</Button>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="boutique" className="mt-6">
          <Card>
            <CardHeader><CardTitle>Boutique</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="shop-name">Nom</Label>
                <div className="relative">
                  <Building2 className="absolute left-3 top-2.5 size-4 text-muted-foreground" />
                  <Input id="shop-name" value={shop?.name ?? ""} onChange={(e) => setShop((current) => current ? { ...current, name: e.target.value } : current)} className="pl-9" />
                </div>
              </div>
              <div className="space-y-2">
                <Label htmlFor="shop-address">Adresse</Label>
                <Input id="shop-address" value={shop?.address ?? ""} onChange={(e) => setShop((current) => current ? { ...current, address: e.target.value } : current)} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="shop-phone">Téléphone</Label>
                <Input id="shop-phone" value={shop?.phone ?? ""} onChange={(e) => setShop((current) => current ? { ...current, phone: e.target.value } : current)} />
              </div>
              <Button onClick={() => void saveShop()}><Save className="size-4" />Enregistrer</Button>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="whatsapp" className="mt-6">
          <Card>
            ...
            <CardContent className="space-y-3">
              {templates.length === 0 ? <p className="text-sm text-muted-foreground">Aucun template enregistré.</p> : templates.map((template) => (
        ...
        <TabsContent value="danger" className="mt-6">
          <Card className="border-destructive/30 bg-destructive/5">
            <CardHeader><CardTitle className="text-destructive">Zone dangereuse</CardTitle></CardHeader>
            <CardContent className="space-y-4">
