...
function ProfilPage() {
  ...
  useEffect(() => {
    if (!user) return;
    form.reset({
      nom: profile?.nom ?? (user.user_metadata["full_name"] as string | undefined) ?? "",
          ...
          <Card>
            ...
            <CardContent>
              {loadingShops ? (
                ...
                <p className="text-sm text-muted-foreground">Aucune boutique associée.</p>
              ) : (
                <div className="grid gap-3 sm:grid-cols-2">
                  {shops.map((shop) => (
                    <div key={shop.id} className="flex items-center gap-3 rounded-lg border p-3">
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      Cette action est irréversible. La suppression complète doit être exécutée par
                      une fonction serveur Supabase sécurisée.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    <AlertDialogCancel>Annuler</AlertDialogCancel>
                    <AlertDialogAction onClick={() => void requestAccountDeletion()}>
                      Confirmer
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
