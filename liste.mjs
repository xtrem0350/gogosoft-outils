...
try {
  ...
  try {
    ...
    console.error(
      '❌ Erreur lors de l\'installation automatique de "glob" :',
...
if (globFn) {
  ...
  if (oldStructureFiles.length > 0) {
    console.log(
      `🧹 Suppression de ${oldStructureFiles.length} ancien(s) fichier(s) de structure...`,
    );
  ...
  // Sans glob
  try {
    const oldStructureFiles = fs
      .readdirSync(rootDir)
      .filter((f) => f.startsWith("project_structure") && f.endsWith(".txt"));
    if (oldStructureFiles.length > 0) {
      console.log(
        `🧹 Suppression de ${oldStructureFiles.length} ancien(s) fichier(s) de structure...`,
      );
